 ...
 **/
public class PREMem implements LocalTransformer {
	 ...
	 **/
	public PREMem(SsaEnvironment env, SsaSymTab sstab) {
		this.env = env;
		this.sstab = sstab;
	}
	
	public PREMem(SsaEnvironment env, SsaSymTab sstab, Function f) {
		this.env = env;
		this.sstab = sstab;
		this.f = f;
	}
	...
	public boolean doIt(Function func, ImList args) {
		f = func;
		env.println("****************** doing exhaustive PRE whose target is mem expression only to " + f.symbol.name, SsaEnvironment.MinThr);
		invoke();
		f.flowGraph().touch();
		return (true);
	}
